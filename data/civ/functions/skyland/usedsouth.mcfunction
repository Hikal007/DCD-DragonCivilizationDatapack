...
tellraw @a[tag=insky] [{"text": " "}]
tellraw @a[tag=insky] [{"text": "【天空之境 - 神秘之源】","color": "red","bold": true},{"text": "能量塔 - 南部 成功激发！","color":"white","bold":false}]
tellraw @a[tag=insky] [{"text": "  能量正在被射向空中！让子弹飞一会！距离下次充能完毕时间为60s！","color":"white","bold":false}]
tellraw @a[tag=insky] [{"text": " "}]
tellraw @a[tag=insky] [{"text": "--------------------------------","color": "red","bold": true}]
tellraw @a[tag=insky] [{"text": " "}]
setblock 0 -58 177 minecraft:stone
...
schedule function civ:skyland/button/buttonwest 60s
